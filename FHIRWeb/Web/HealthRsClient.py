...
def get_fhir_resource_list(rs_host, rs_port, fhir_res_type, access_token):
    url = 'http://{}:{}/fhir/{}'.format(
        rs_host, rs_port, fhir_res_type)

    header = {
        'Access-Token': access_token
    }

    Log.debug('[get_fhir_resource_list]\
        \n\turl: {}\
        \n\theader: {}'.format(url, header))
...
def share_res(rs_host, rs_port, fhir_res_type, fhir_res_id, access_token, target_user):
    url = 'http://{}:{}/share/{}/{}'.format(
        rs_host, rs_port, fhir_res_type, fhir_res_id)
    ...
    Log.debug('[share_res]\
        \n\turl: {}\
        \n\theader: {}\
        \n\tpayload: {}'.format(url, header, data))

    return requests.post(url=url, headers=header, json=data)


def unshare_res(rs_host, rs_port, fhir_res_type, fhir_res_id, access_token, policy_id):
    url = 'http://{}:{}/share/{}/{}'.format(
        rs_host, rs_port, fhir_res_type, fhir_res_id)
    ...
    Log.debug('[unshare_res]\
        \n\turl: {}\
        \n\theader: {}\
        \n\tpayload: {}'.format(url, header, data))

    return requests.delete(url=url, headers=header, json=data)