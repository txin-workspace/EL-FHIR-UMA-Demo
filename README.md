...
1. **リソースシェア**
	- FHIRリソースサーバからDemoAppにシェア
		1. FHIRのWeb管理画面にアクセス
		2. 画面に従ってログイン <br>  **一回ログインすると、トークンの有効期限は300s。<br>  期限切れ後、シェアはできなくなり、ブラウザリフレッシュをする必要がある**
		3. 「share this resource」ボタンをクリックする
		4. 「domain-name:user-name」の形式でシェア対象を指定する
		5. 「Share!」ボタンをクリックするとシェアが成立する <br>  **シェアが成功した場合、画面は自動的にRefreshする**
...
1. DemoAppの画面(`:11000`)で、health-helperのアカウントでApp認証サーバにログインしてからOICDでEL認証サーバにログインする`app-keycloak-oidcボタン`。何もシェアされ得ていないため、画面にリソース情報は表示されていない。
2. FHIRのWeb管理画面(`:12000`)で、pcha-data-ownerのアカウントでFHIR認証サーバにログインする。res_uploadによって生成されたリソースが表示されている。リソースのシェアボタンをクリックして`rs-elwebapi:health-helper`を入力し、確定ボタンを押す。
3. DemoAppの画面をリフレッシュすると、先ほどシェアされたFHIRリソースが画面に表示される。
4. ELのWeb管理画面(`:13000`)で、el-data-ownerのアカウントでEL認証サーバにログインする。res_uploadによって生成されたリソースが表示されている。リソースのシェアボタンをクリックして`health-helper`を入力し、確定ボタンを押す。
5. DemoAppの画面をリフレッシュすると、先ほどシェアされたELリソースが画面に表示される。
6. Postmanでで先ほどシェアされたリソースの値を更新する。
7. DemoAppの画面をリフレッシュすると、該当リソースの値も更新される。
8. FHIRのWeb管理画面で先ほどシェアされたリソースをunshareボタンを押す。
9. DemoAppの画面をリフレッシュすると、該当リソースが画面に表示されなくなる。
