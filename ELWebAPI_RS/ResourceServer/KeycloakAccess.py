...
def token_get_uname(header_auth: str):
    response = UmaClient.keyclack.check_token(
        rs_config.am_host, rs_config.am_port, rs_config.realm_name,
        rs_config.client_id, rs_config.client_screct, 
        header_auth.replace('Bearer ',''))
    ...
    return True, response.json()['username'], response.json()['azp']
...
def share_resource_with_name(header_auth: str, res_id, share_type, target_user, policy_name):
    if share_type == 'user':
        response = UmaClient.keyclack.policy_create_with_name(
            rs_config.am_host, rs_config.am_port, rs_config.realm_name,
            header_auth.replace('Bearer ',''), res_id, 'users', target_user, policy_name)

    elif share_type == 'client':
        response = UmaClient.keyclack.policy_create_with_name(
            rs_config.am_host, rs_config.am_port, rs_config.realm_name,
            header_auth.replace('Bearer ',''), res_id, 'clients', target_user, policy_name)
...
def user_logout(refresh_token):
    response = UmaClient.keyclack.user_logout(
        rs_config.am_host, rs_config.am_port, rs_config.realm_name,
        rs_config.client_id, rs_config.client_screct, refresh_token)
